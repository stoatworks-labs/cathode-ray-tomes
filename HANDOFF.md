...
**Asteroids takes the parts lists now, inside the generator.** It was declined
twice on two grounds, and only one of them held. The first — a merge into the
emitted `boards/asteroids-0*.json` is lost on regeneration — was right, and the
supplement now runs as the last step of `build_asteroids_revisions.py` so it
survives. The second — that the ten printings span revisions whose designators
shift, so merging them manufactures false disagreements — was checked and is
false: scored against the 28 devices whose early and late designators the drawing
read both records, every printing writes *early* designators (18–22 early against
0–1 late), the 6th printing alone hedging at 18 to 10. The shift lives in the
boards, not the manuals. So the nine unambiguous printings merge onto -03 and -04
(+22 each, 8 disputed with the drawing kept and the dispute noted per device), and
-05/-06 take nothing: a parts-list device would land one column from where those
boards actually have it, and for the 28 with a known alternate would duplicate a
chip the drawing already placed. They keep the caveat they carry rather than gain a
quieter one.
